...
class Centrality:
    ...
    def degree_centrality(self, name, orientation="NATURAL"):
        '''
        Wrapper for 
    ...
    def article_rank(self, name):
        ...
        res = self._driver.run_cypher(qry)
...
class CommunityDetection:
    ...
    def average_lcc(self,name):
        name = _normalise_gn(name)
        qry = self._qry_builder.average_lcc(name)
        return self._driver.run_cypher(qry)
    ...
    def triangle_count(self, name,mode="stream"):
        ...
        '''
        name = _normalise_gn(name)
        qry = self._qry_builder.triangle_count(name,mode=mode)
        return self._driver.run_cypher(qry)
...
class PathFinding:
    ...
    def all_shortest_path(self, name, mode="stream"):
        name = _normalise_gn(name)
        qry = self._qry_builder.all_shortest_paths(name, mode)
        return self._driver.run_cypher(qry)
    ...
    def bfs(self, name, source, dest=None,
            max_depth=None, mode="stream"):
        name = _normalise_gn(name)
        qry = self._qry_builder.bfs(name, source, dest,
                                    max_depth, mode)
        return self._driver.run_cypher(qry)
...
class Procedures():
    def __init__(self, interface):
        self._builder = GDSQueryBuilder()
        self._interface = interface
        self.centrality = Centrality(interface,self._builder)
        self.community_detection = CommunityDetection(interface,self._builder)
        self.similarity = Similarity(interface,self._builder)
        self.path_finding = PathFinding(interface,self._builder)
        self.tpp = TPP(interface,self._builder)
        self.modes = ["stream","mutate","stats","write"]

    def stats(self,name):
        qry = self._builder.stats(name)
        return self._interface.run_cypher(qry)
        
    def edge_stream(self,name):
        qry = self._builder.edge_stream(name)
        return self._interface.run_cypher(qry)
